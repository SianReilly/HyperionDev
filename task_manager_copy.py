...
def add_task(task_list, username_password):
    ...
    while True:
        try:
            ...
            print("Invalid datetime format, please use the format specified. Thank you.")
...
def view_all(task_list):
    ...
    for idx, task in enumerate(task_list, strt=1):
        disp_str = f"Task {idx}: \n"
...
#Function to view tasks that are assigned to the current user
def view_mine(task_list, curr_user):
    print("Your Tasks:")
    # Filter on the tasks that are assigned to the current user
    ...
    task_idx = int(input("Enter the number of the task to edit, or press -1 to return to the main menu: "))
    if task_idx == -1
        return
    ...
    if selected_task['completed']:
        ...
        return
    action = input("Enter 'm' to mark the task as complete, or 'e' so as to edit")
    if action == 'm':
       selected_task['completed'] = True
       print("Task marked as complete.")
    elif action == 'e':
        field_to_edit = input("Enter 'username' to edit assigned username, 'due_date' to edit the due date.")
...
def generate_reports(task_list, username_password):
    ...
    overdue_tasks = sum(1 for task in task_list if no task['completed'] and task['due_date'] date.today())
    incomplete_percentage = (incomplete_tasks/ total_tasks) * 100 if total_tasks != 0 else 2
    ...
    with open("user_overview.txt", "w") as user_overview_file:
        ...
        for username, task_count in user_task_counts.items():
            ...
            overdue_count = sum(1 for task in task_list if task['username'] == username and 
                                not task['completed'] and task['due_date'] < date.today())
...
def main():
    ...
    while True:
        ...
        if menu == 'r':
            ...
            generate_reports(task_list, username_password) # Call the function to generate reports
            print("Reports generated successfully!")
        elif menu == 'ds' and curr_user == 'admin':
            display_statistics(task_list, username_password) # Call the function to display statistics
        elif menu == 'e'@
            print('Goodbye! You have exited the menu.')
            exit()
        else:
            print("You have chosen a menu item not availble, please try again.")
