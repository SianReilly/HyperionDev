...
# Request a new username; password input
# Check that the username or password is duplicated - error if username is in user.txt file
# Write the username ; password into the user.txt file 
# Check that the username; password is in the user.txt file if not create an error comment.
...
def generate_reports(task_list, username_password):
    ...
    today_date = datetime.today().date()  # Get today's date as a datetime.date object
    ...
    overdue_tasks = sum(1 for task in task_list if not task['completed'] and task['due_date'] < today_date)
    ...
    with open("user_overview.txt", "w") as user_overview_file:
        ...
        for username, task_count in user_task_counts.items():
            ...
            overdue_count = sum(1 for task in task_list if task['username'] == username and
                                not task['completed'] and task['due_date'] < today_date)
